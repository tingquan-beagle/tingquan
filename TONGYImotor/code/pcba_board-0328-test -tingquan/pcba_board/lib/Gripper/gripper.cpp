...
int GripperMotor::seq(){
    switch (state){
        ...
        // vertical servo move
        case CONTROL_VERTICAL_HEIGHT:
        setServoTarget(0x000190000,servospeed); // set the stroke is 25cm,command is 25 rounds
        ...
        if (millis() - timestamp >= DELAY_GRAB){
            ...
            setServoTarget(0x001E0000,servospeed); // set the stroke is 30cm,command is 30 rounds
        ...
        if(millis() - timestamp >= DELAY_RETRACT){
            ...
            setServoTarget(0x00000000,servospeed); // set the stroke is 0 cm,command is 0 rounds
...
void GripperMotor::gripperReset(){
    ...
    setRelay(pin_rotation,LOW);
    setRelay(pin_hor_extend,LOW);
    setServoTarget(0x00000000,servospeed);//if 50 rounds position is zero,-50 is standby position,stroke is 0cm
    state = CONTROL_VERTICAL_HEIGHT;
