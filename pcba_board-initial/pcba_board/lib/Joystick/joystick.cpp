...
void Joystick::controlCylinders() {
    ...
    if (is_left_manual_mode && !is_right_manual_mode) {
        // Check if left vertical control is engaged and in automatic mode
        if (is_left_control_engaged) {
            // Calculate the position change based on the mapped input and divide by scaling factor
            float position_change_y = handle_y / SCALING_FACTOR;

            // Set the accumulated position on the left vertical cylinder using the setVal() function
            leftVerticalCylinder->setPosition(int(position_change_y));
        }

        // Otherwise, send a command value of 0 to bring the spool position back to 0.
        else {
            leftVerticalCylinder->setPosition(0);
        }

    }

    // If it is in automatic mode, do automatic control
    else if (is_left_automatic_mode) {
        leftVerticalCylinder->updateAuto(ver_left_cmd_val / SCALING_FACTOR);
    }

    // Otherwise, send a command value of 0 to bring the spool position back to 0, because the left switch is in a neutral mode.
    else {
        leftVerticalCylinder->setPosition(0);
    }
    
    // Check if the control and deadman switch are engaged for right control
    if (is_right_manual_mode && !is_left_manual_mode) {
        // Check if right vertical control is engaged and in automatic mode
        if (is_right_control_engaged) {
            // Calculate the position change based on the mapped input and divide by scaling factor
            float position_change_y = handle_y / SCALING_FACTOR;

            // Set the accumulated position on the right vertical cylinder using the setVal() function
            rightVerticalCylinder->setPosition(int(position_change_y));
        }

        // Otherwise, send a command value of 0 to bring the spool position back to 0.
        else {
            rightVerticalCylinder->setPosition(0);
        }
    }

    // If it is in automatic mode, do automatic control
    else if (is_right_automatic_mode) {
        rightVerticalCylinder->updateAuto(ver_right_cmd_val / SCALING_FACTOR);
    }

    // Otherwise, send a command value of 0 to bring the spool position back to 0, because the right switch is in a neutral mode.
    else {
        rightVerticalCylinder->setPosition(0);
    }
    ...
    if (is_right_manual_mode || is_left_manual_mode ) {
        ...
        if (is_right_control_engaged || is_left_control_engaged ) {
            
            float position_change_x = handle_x / SCALING_FACTOR;

            if (position_change_x > 50) {
            pinMode(RELAY_3,OUTPUT);
            pinMode(RELAY_4,OUTPUT);
            digitalWrite(RELAY_3,HIGH);
            digitalWrite(RELAY_4,LOW);
            }
            else if (position_change_x < -50) {
            pinMode(RELAY_3,OUTPUT);
            pinMode(RELAY_4,OUTPUT);
        ...
        else if (is_deadman_engaged) {

        float position_change_y = handle_y / SCALING_FACTOR;
    
            if (position_change_y > 50) {
            
            // Serial.print("user wants to make the pruner higher");
            pinMode(RELAY_1,OUTPUT);
            pinMode(RELAY_2,OUTPUT);
            digitalWrite(RELAY_1,HIGH);
            digitalWrite(RELAY_2,LOW);
            }
            else if (position_change_y <-50) {
            
            // Serial.print("user wants to make the pruner lower");
            pinMode(RELAY_1,OUTPUT);
            pinMode(RELAY_2,OUTPUT);
